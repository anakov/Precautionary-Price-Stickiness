...
VarPrice=sum((log(PMAT(:))-ElogPrice).^2.*Pdist(:));                 % variance of prices (logs)
...
%%%%%%%%%%%%%%%%%%%%%
U = Cbar^(1-gamma)/(1-gamma) - chi*Nbar; 
%%%%%%%%%%%%%%%%%%%%%
...
if maxD>minD && Dcutoff>minD                                         % prepare Lambda as a function of D graph 
   ...
   Lvalues = adjustment(adjtype, Dgrid'/wbar, ksi, alpha, lbar, sum(sum(adjcost.*Pdisteroded)));
   Lvalues2 = adjustment(adjtype, Dgrid2'/wbar, ksi, alpha, lbar, sum(sum(adjcost.*Pdisteroded)));
end
...
lbound = PMIN+pstep/2;
hbound = PMAX-pstep/2;
%edges=[-inf linspace(lbound, hbound, 2*nump-2) inf];                         
edges=[-inf -0.49:0.02:0.49 inf];                         
...
denspchanges=prob;                % for compatibility with pps when estimating (see criterion in "distance.m")
freqnonzeropchanges=freqpchanges; % same as above

freqdata = 0.1;
Pchangegrid=((edges(2:end)+edges(1:end-1))/2)';
midricalc;
...
FlexPriceDispersion=sum(sum(sMAT.*(PMAT.^(-epsilon)).*ergodicDistFlexPrice));       % Productivity-weighted D-S dispersion      