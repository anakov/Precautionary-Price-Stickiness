...
disp(sprintf('Matrices loaded. Elapsed time: %d seconds', round(toc)))  
disp('Start QZ decomposition ... (this takes a few minutes)') % a.k.a. generalized Schur decomposition
A=full(A);
B=full(B);
...
disp(sprintf('\n'))  
disp(sprintf('QZ decomposition finished. Elapsed time: %d seconds', round(toc)))
disp('Start OrdQZ ...')
...
disp(sprintf('\n'))  
disp(sprintf('ORDQZ finished. Elapsed time: %d seconds', round(toc)))
disp('Start z11 rank check ...')
...
disp(sprintf('\n'))  
disp(sprintf('z11 rank check finished. Elapsed time: %d seconds', round(toc)))
disp('Start consistency checks ...')
...
if (checkklein > 0.00001 || checkklein2 > 0.00001 || checkklein3 > 0.00001 )
    disp(sprintf('\n'))  
    toc
    ...
    disp(sprintf('\n'))  
    disp(sprintf('Consistency checks: OK'))
    disp(sprintf('Total elapsed time: %d seconds', round(toc)))
end
